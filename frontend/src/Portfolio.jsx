...
const Portfolio = () => {
  ...
  const NavLink = ({ id, icon: Icon, label }) => (
    <button
      ...
      className={`relative flex items-center space-x-2 px-4 py-2 rounded-full transition-all duration-300 group ${activeSection === id
          ? 'text-white'
          : 'text-gray-400 hover:text-white'
        }`}
    >
      <div className={`absolute inset-0 rounded-full bg-gradient-to-r from-blue-600 to-purple-600 opacity-0 group-hover:opacity-100 transition-opacity ${activeSection === id ? 'opacity-100' : ''
        }`} />
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-gray-100 font-sans selection:bg-blue-500/30 selection:text-white overflow-x-hidden">
      ...
      {/* Navigation */}
      <nav className={`fixed top-0 w-full z-50 transition-all duration-500 ${isScrolled ? 'bg-gray-950/90 backdrop-blur-xl py-3 border-b border-gray-800/50' : 'bg-transparent py-5'
        }`}>
            ...
            <div className="flex flex-col p-4 gap-2">
              {['home', 'skills', 'experience', 'projects', 'contact'].map((section) => (
                <button
                  ...
                  className={`flex items-center gap-3 px-4 py-3 rounded-xl text-left transition-all ${activeSection === section
...
const ProjectCard = ({ title, tags, description, featured, delay, link, icon }) => (
  <div
    className={`group glass-card border p-6 rounded-2xl transition-all duration-300 hover:scale-[1.02] h-full animate-on-scroll ${featured
  ...
  >
    ...
    <h3 className={`text-2xl font-bold mb-4 text-white group-hover:text-gradient transition-all duration-300 ${featured ? 'text-gradient' : ''
      }`}>
      {title}
    </h3>
    ...
    <div className="flex flex-wrap gap-2 mt-auto">
      {tags.map((tag, i) => (
        <span
          key={i}
          className={`text-xs px-3 py-1.5 rounded-full font-medium border ${featured
    ...
    {link && (
      <div className="mt-6 pt-6 border-t border-gray-800">
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className={`text-sm font-medium flex items-center gap-2 ${featured ? 'text-purple-400 hover:text-purple-300' : 'text-blue-400 hover:text-blue-300'
            }`}
        >
          View Project
